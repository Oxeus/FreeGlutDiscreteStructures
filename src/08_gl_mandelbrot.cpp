...
int main(int argc, char** argv) {
	cout << "Remember that the key to change the zoom is Z and the key to change the iteration factor is L." << endl;
	cout << "The equation is M = M^Q + C^R. All you need to do is enter the values for Q and R. " << endl;
	cout << "Please enter a value for Q: ";
	cin >> Q;
...
void fill(unsigned char arr[ARESY][ARESX][3]) {
	for(int r = 0; r < ARESY; r++) {
		for(int c = 0; c < ARESX; c++) {
			...
			int n = isBounded(C);
			arr[r][c][0] = 5*n%256; // red
			arr[r][c][1] = 10*n%256; // green
			arr[r][c][2] = 15*n%256; // blue
		}
	}
}
